    ...
    <section id="skills" className="relative pt-24 md:pt-32 pb-20 md:pb-32 overflow-hidden bg-slate-50 dark:bg-[#09090b] transition-colors duration-300">
      ...
      <div className="absolute top-0 left-0 w-full overflow-hidden leading-[0] z-0">
        <svg viewBox="0 0 1200 120" preserveAspectRatio="none" className="relative block w-full h-[60px] md:h-[120px]">
          <path d="M0,0V46.29c47.79,22.2,103.59,32.17,158,28,70.36-5.37,136.33-33.31,206.8-37.5C438.64,32.43,512.34,53.67,583,72.05c69.27,18,138.3,24.88,209.4,13.08,36.15-6,69.85-17.84,104.45-29.34C989.49,25,1113-14.29,1200,52.47V0Z" className="fill-white dark:fill-white/[0.12] transition-colors duration-300" opacity=".25"></path>
          <path d="M0,0V15.81C13,36.92,27.64,56.86,47.69,72.05,99.41,111.27,165,111,224.58,91.58c31.15-10.15,60.09-26.07,89.67-39.8,40.92-19,84.73-46,130.83-49.67,36.26-2.85,70.9,9.42,98.6,31.56,31.77,25.39,62.32,62,103.63,73,40.44,10.79,81.35-6.69,119.13-24.28s75.16-39,116.92-43.05c59.73-5.85,113.28,22.88,168.9,38.84,30.2,8.66,59,6.17,87.09-7.5,22.43-10.89,48-26.93,60.65-49.24V0Z" className="fill-white dark:fill-white/[0.12] transition-colors duration-300" opacity=".5"></path>
          <path d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z" className="fill-white dark:fill-white/[0.12] transition-colors duration-300"></path>
        </svg>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <motion.div 
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="md:col-span-2 bg-white dark:bg-white/[0.02] border border-slate-200/80 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-2xl rounded-[32px] p-8 md:p-12 relative overflow-hidden group hover:border-sky-300 dark:hover:border-sky-500/30 transition-colors"
          >
          ...
          <motion.div 
            ...
            className="md:col-span-1 bg-white dark:bg-white/[0.02] border border-slate-200/80 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-2xl rounded-[32px] p-8 md:p-12 relative overflow-hidden group hover:border-emerald-300 dark:hover:border-emerald-500/30 transition-colors"
          >
          ...
          <motion.div 
            ...
            className="md:col-span-1 bg-white dark:bg-white/[0.02] border border-slate-200/80 dark:border-white/5 shadow-xl shadow-slate-200/50 dark:shadow-2xl rounded-[32px] p-8 md:p-12 relative overflow-hidden group hover:border-indigo-300 dark:hover:border-indigo-500/30 transition-colors"
          >
          ...
          <motion.div 
            ...
            className="md:col-span-2 bg-slate-900 border border-slate-800 dark:bg-white/[0.02] dark:border-white/5 shadow-xl rounded-[32px] p-8 md:p-12 relative overflow-hidden group hover:border-orange-500/50 transition-colors"
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-8 justify-between items-center h-full">
              ...
              <div className="w-full md:w-1/2 flex flex-wrap justify-center md:justify-end gap-6">
                <TechIcon folder="amazonwebservices" file="amazonwebservices-original-wordmark.svg" name="AWS" isOld lightText darkDropShadow />
                <TechIcon folder="docker" file="docker-original.svg" name="Docker" isOld lightText />
                <TechIcon folder="github" file="github-original.svg" name="GitHub" isOld lightText invertInDark />
...
const TechIcon = ({ folder, file, name, featured, isOld, lightText, invertInDark, darkDropShadow }) => {
  const baseUrl = isOld 
  ...
  return (
    <div className={`flex flex-col items-center justify-center gap-3 ${featured ? 'w-24 h-24' : 'w-16 h-16'}`}>
      <div className={`bg-white dark:bg-white/5 border border-zinc-100 dark:border-white/10 flex items-center justify-center rounded-2xl shadow-sm dark:shadow-none transition-transform duration-300 hover:scale-110 hover:-translate-y-1 ${
        featured ? 'w-20 h-20 p-4 border-zinc-200' : 'w-14 h-14 p-3'
      }`}>
        <img src={`${baseUrl}/${folder}/${file}`} alt={name} className={`w-full h-full object-contain drop-shadow-sm ${invertInDark ? 'dark:invert' : ''} ${darkDropShadow ? 'dark:drop-shadow-[0_0_2px_rgba(255,255,255,0.8)]' : ''}`} />
      </div>
      <span className={`font-medium ${featured ? 'text-base' : 'text-xs'} ${lightText ? 'text-zinc-300' : 'text-slate-600 dark:text-zinc-300'}`}>{name}</span>
    </div>
  );
};
...
const TechRow = ({ folder, file, name, isOld }) => {
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-white dark:bg-white/5 border border-zinc-100 dark:border-white/10 rounded-2xl shadow-sm dark:shadow-none hover:shadow-md hover:-translate-y-0.5 transition-all">
      <div className="w-10 h-10 p-2 bg-zinc-50 dark:bg-white/10 rounded-lg flex items-center justify-center">
        <img src={`${baseUrl}/${folder}/${file}`} alt={name} className="w-full h-full object-contain" />
      </div>
      <span className="font-semibold text-zinc-800 dark:text-white">{name}</span>
    </div>
  );
};
